...
- **Project Explorer** - Browse project file trees with syntax-aware icons and color-coded file types (40+ file extensions). Auto-refreshes on file system changes
...
- **Usage Tracking** - Monitor daily Claude API usage (messages, tool calls, sessions) with a 14-day chart view
...
- **プロジェクトエクスプローラー** - ファイルツリーを構文対応のアイコンと色分けで表示（40種類以上のファイル拡張子対応）。ファイルシステム変更時に自動リフレッシュ
- **スニペットパネル** - コードスニペットを保存し、アクティブなコンソールにワンクリックで送信（テキスト中の `\r` で Enter キーを送信）。ドラッグ＆ドロップによる並べ替えに対応
- **使用量トラッキング** - Claude API の日次使用量（メッセージ数、ツールコール数、セッション数）を14日間のチャートで表示
